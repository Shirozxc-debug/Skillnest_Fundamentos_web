// 🎯 Objetivo
// Aplicar el uso de bucles y condicionales para resolver problemas computacionales mediante la implementación de algoritmos en JavaScript,
//  reforzando los conocimientos adquiridos en los capítulos anteriores.

// ❓ ¿Por qué es importante?
// Los bucles son una herramienta fundamental en programación, permitiendo ejecutar tareas repetitivas de manera eficiente.
//  Comprender cómo usarlos correctamente mejora la optimización del código y facilita la automatización de procesos.

// 📋 Instrucciones 
// Resuelve cada ejercicio utilizando for o while, según corresponda. Recuerda probar tu código con console.log() para verificar los resultados.
...
//  Escribe un programa que imprima los números del 1 al 10 para simular las interacciones de un usuario. Ejemplo de salida esperada:
// Interacción 1
// Interacción 2
// ...
// Interacción 10​​
...
function contarInteracciones() {
    let suma = 0
    for (let i = 1; i <= 10; i++)
        alert(`Ïnteracción ${i}: y suma es ${suma}`)
}

// ⏳ Simular una barra de carga: Crea un programa que simule una barra de carga mostrando porcentajes de progreso del 0% al 100% en incrementos de 10.
...
// 🛍️ Simulación de descuento progresivo en una tienda: Una tienda en línea ofrece un descuento progresivo en una promoción especial.
//  El primer producto tiene un 5% de descuento,
//  el segundo un 10%, el tercero un 15%,
...
//  Valor total con desuento = 47500
...
function DescuentoTienda() {
    ...
    for (let i = 5; i <= 25; i += 5) {
        let descuento = (ValorProducto * i) / 100;
        let ValorTotal = ValorProducto - descuento;
        let numeroProducto = i / 5;
        alert(`\nProducto: ${numeroProducto}
            \nDescuento aplicado: ${i}% 
            \nValor total con descuento: $${ValorTotal}`);
    }
}
...
function totalDePuntos() {
    for (let nivel = 1; nivel <= 5; nivel++) {
        let puntos = nivel * 50
        alert(`Nivel ${nivel}: ${puntos} puntos`)
    }
}
...
//  Escribe un programa que muestre un mensaje con la cantidad de notificaciones pendientes y las marque como leídas una por una hasta llegar a 0.
//  Ejemplo de salida esperada:
...
function notificaciones() {
    for (let notis = 5; notis >= 0; notis--) {
        if (notis <= 0) {
            ...
            alert(`Tienes ${notis} notificaciones nuevas.`)
        }
    }
}
...
//  en distintos escenarios prácticos, aplicando la lógica de programación de manera eficiente.
// Estos ejercicios reflejan situaciones comunes en el desarrollo de software, ayudándote a automatizar tareas repetitivas y mejorar la optimización del código.